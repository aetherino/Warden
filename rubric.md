...
* **Models (tiering):** think-heavy **adjudication** runs on the **strongest model (Claude Opus 4.8, `claude-opus-4-8`)** — the §7 prosecutor/skeptic/adjudicator confidence pipeline and the §11 search-grounded pathway judge, where reasoning quality is the correctness bottleneck. Latency-bound / high-volume calls may use a faster model (runtime triage + labeling on the **≤8s hot path**, §A). This is only affordable because the heavy adjudication is **precomputed off the runtime hot path** (§7/§10) — so the upgrade buys correctness without spending runtime budget. Model IDs are configurable (`WARDEN_*_MODEL`).
...
13. **Live agentic scan (§12):** the runtime streams schema-valid step events (≥1 per source as it resolves) + a terminal dossier event, and the UI renders a live scan log of those **real** steps before the final dossier — no fabricated delays; the clean/empty path streams too (never a blank wait, never an all-clear).
14. **Deployment (§D):** the app is live at a public **Vercel production URL** serving the real UI (not just a preview); the Agent SDK brain is reachable from it (Vercel Python function *or* tunneled self-host) and `/api/dossier` + the §12 stream proxy through to it end-to-end; **all secrets come from Vercel env** (ANTHROPIC/EXA/OPENALEX + DB + any Clerk keys), never the repo; the store is the **deploy Postgres backend**, not the dev SQLite file. Smoke-checked on the **LIVE URL** (Chrome plugin drives production, not localhost): seeded basket yields ≥1 of each tier with every citation re-fetching 200, §12 streams, empty-input graceful, all within the §A budget.
...
* **Shield = telemetry, not decoration (the "invisible shield" — active build item, #047 / spec `design/SHIELD_FX.md`):** the centerpiece is **driven by the real §12 `ScanEvent` stream**, not a generic spinner — each source-check step pulses a **sonar-ping ripple** across the shell (the user literally watches the agent sweep its sensors), so *loading is the live audit rendered*. It must read as a **shield, not a ball**: a **Fresnel membrane** (rim-lit translucent dome) around a **negative-space core** (the protected home/person), with an **ACT breach-rupture** as the lone hard-alarm motion. Loading→outcome is **one continuous settle** — the field redistributes into the result tier, never a spinner-then-cut. All prior constraints still bind: **tier-only, origin-blind, never a safety verdict, no green/all-clear even when calm**; implemented as shader uniforms + per-particle attributes on the existing `THREE.Points`, device-aware count, static fallback untouched.
...
## **§12 Live agentic scan (run visibility)**

The autonomy must be **visible**, not only stored. The runtime **streams its real steps as they happen** — per source, per item, plus the context-driven sources — and the UI renders a live **scan log** before the ranked dossier.

* **Streamed, honest steps:** the brain exposes a streaming endpoint (SSE / chunked) emitting a typed step event as each unit of work completes — e.g. *"CPSC · space heater → 6 recalls → triaging…"*, *"space heater → ACT: fire hazard \[cited]"*, *"ZIP 48503 → water system FLINT, CITY OF → 1 SDWA violation → ADDRESS"*, *"Prop 65 · extension cord → 19 notices → CONTEXT (suppressed)"* — then a terminal event carrying the final dossier. Events reflect work the agent **actually did**; **no fabricated steps or theatrical delays**.
* **Event shape:** `{seq, phase, source, item?, status: started|done|empty|error, detail, tier?}`; terminal `{type:"dossier", ...}` (the same dossier §2 returns).
* **UI:** a live activity/scan list during the run; the shield reacts to the **real stream** — each streamed step pulses it (a §10 sonar ping) and loading→outcome is one continuous settle, never a spinner-then-cut; then the ranked dossier replaces the log (the scan stays available, collapsed). The clean/empty path streams too ("checked \[sources\]… nothing on file as of \<date\>") — never a blank wait, never an all-clear.
* **Budget + honesty (ties to §A/§10):** streaming only surfaces steps already happening inside the ≤8s runtime budget — it adds no latency. It **complements** the stored, inspectable §10 per-claim record (the deep build-time adversarial/confidence + §11 pathway-judge pipeline): the live scan visualizes the runtime resolve; the §10 record is the deep audit trail. Together they make Autonomy + Orchestration legible to a human watching AND to a verifier re-running the gates.
...
"Done" includes *shipped*. The build→verify loop and the demo both run against a live Vercel deployment, not localhost.

* **UI:** Next.js App Router deployed to **Vercel Production** (Fluid Compute default; Node 24). Preview deploys per change are first-class verifiers (live-URL checks, not only unit tests).
* **Brain:** the Python Agent SDK harness is reachable from the deployment — either a **Vercel Python function** (Fluid Compute supports Python natively) or a **self-hosted brain exposed via cloudflared tunnel**, with the UI's `WARDEN_SERVICE_URL` pointed at it. `/api/dossier` and `/resolve/stream` (§12) must work end-to-end against the live brain.
* **Store:** swap the dev **SQLite** for **Postgres** (Supabase or Vercel-Marketplace **Neon** — Vercel no longer ships a first-party DB) behind the existing store module; the cache survives cold starts.
* **Secrets:** every key via **Vercel env** (Production + Preview scopes); none committed; `.env` stays gitignored. Verifier greps the deployed bundle/repo for leaked keys → must find none.
* **Auth:** Clerk stays **flag-gated** — present `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` in Vercel env → auth is live and gates the audit + the MCP/API surface (#045); absent → inert, same tree as today.
* **Budget:** the platform 300s function ceiling does **not** relax the §A **≤8s** turn-cap/fallback — that still governs the UX and is verified on the live URL.
* **Smoke (Gate 14):** the production URL is the artifact the §8 verifier and the §E T-30min check run against.
...
Impact → §4–§7 (prioritized, caveated, actionable). Demo → §2 + §5 decoys + §9 robustness states + §10 shield + **§12 live scan** + no-hang fallback. Autonomy → §8 loop + self-caught failures in the log + **the §12 live scan dramatizes the agent reasoning through each source**. Orchestration → done is machine-checkable here; the harness + this rubric are the judged artifacts; the pipeline is made legible **live via §12** and **inspectable via §10**; swap fixtures/sources to rerun.
