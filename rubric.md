...
* **UI:** a live activity/scan list during the run; the shield reacts as findings land; then the ranked dossier replaces the log (the scan stays available, collapsed). The clean/empty path streams too ("checked \[sources\]… nothing on file as of \<date\>") — never a blank wait, never an all-clear.
